(** 
# Apiary Provider: Apiary 型プロバイダー (試験用)
...
機能するわけではありません。
型プロバイダーは `ApiaryProvider.dll` にあります：
...
この記事ではApiary Providerの試験用機能である `ApiaryProvider` のデモを紹介しました。
 ...
 * [Apiary Provider: JSON Type Provider](../library/JsonProvider.html) -
   JSONドキュメントを処理するような、より単純な型プロバイダーについて
   説明しています。(HTTPリクエスト明示的に行っている場合には)こちらの
   安定した機能を使うとよいでしょう。
 * [Apiary Provider: HTTP Utilities](../library/Http.html) -
